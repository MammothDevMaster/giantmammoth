...
contract Staking is IStaking, InjectorContextHolder {
    ...
    // _validatorSnapshots[address][uint64] = ValidatorSnapshot
    // ValidatorSnapshot = _validatorSnapshots[address][uint64]
    ...
    function _calcValidatorSnapshotEpochPayout(ValidatorSnapshot memory validatorSnapshot) internal view returns (uint256 delegatorFee, uint256 ownerFee, uint256 systemFee) {
        ...
        address validatorAddress = _activeValidatorsList[i];
        Validator memory validator = _validatorsMap[validatorAddress];
        ValidatorSnapshot memory snapshot = _validatorSnapshots[validator.validatorAddress][validator.changedAt];
        totalDelegatedAmount += uint256(snapshot.totalDelegated) * BALANCE_COMPACT_PRECISION;
        }
        uint256 addEcoReward = validatorSnapshot.totalRewards +  ECOSYSTEM_REWARD * validatorSnapshot.totalDelegated / totalDelegatedAmount * BALANCE_COMPACT_PRECISION ;
        ...
        if (validatorSnapshot.slashesCount >= _chainConfigContract.getMisdemeanorThreshold()) {
            ...
            return (delegatorFee = 0, ownerFee = addEcoReward, systemFee = 0);
        }
        // ownerFee_(18+4-4=18) = totalRewards_18 * commissionRate_4 / 1e4
        ownerFee = addEcoReward * validatorSnapshot.commissionRate / 1e4  ;
        // delegatorRewards = totalRewards - ownerFee
        delegatorFee = addEcoReward - ownerFee;
    ...
    function setEcosystemReward(uint256 _amount) external onlyFromGovernance {
        ECOSYSTEM_REWARD = _amount;
    }
